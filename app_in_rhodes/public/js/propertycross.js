...
function misspelledLocation(place_name) {
	$("#error_message").html(place_name).trigger("create");
	$("#recent_search").hide();
}

function fillRecentSearch(recent_search_html) {
	$("#recent_search_list").html(recent_search_html).trigger("create");
}

function showMyLocation(lat_lang) {
...
function showMoreSearchResult(more_search_result) {
...
$(document).ready(function() {
	...
	$(".number_of_li").html($("#search_view_results li").size() - 1);
	$("#go").live('click', searchProperty);
	$('.misspelt_place_li, .recent_search_li').live('click', searchListing)
	$("#fave_plus").live("click", addFavouritePlace);
	$("#fave_minus").live("click", removeFavouritePlace);
	$("#total_number_of_property").live("click", showMoreProperty);

	function searchProperty() {
		var search_text = $("#search_field").val().trim();
		if (search_text.length == 0) {
			showError("Search field should not be empty");
		} else {
			$("#Searching_label").show();
			$("#error_message").html("");
	...
	function searchListing() {
		...
		if (recent_search_by_my_location == "recent_search_by_my_location") {
			...
			var jqxhr = $.post("/app/PropertyCross/search_listings", {
					"place_name": misspelt_place_name
				}, function(data) {
					$("#Searching_label").hide();
				});
		}
	...
	function showMoreProperty() {
		var total_records = $(".number_of_li").html().trim();
		var long_title = $("#long_title").html().trim();
		if (total_records > 19) {
			var jqxhr = $.post("/app/PropertyCross/more_search_result", { "page": total_records.substr(0, 1), "place_name": long_title }, function() { });
		}
	}
});